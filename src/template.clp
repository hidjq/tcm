(deftemplate Disease
    ...
    (multislot recommended-herbs) ; 推荐使用的草药
)
...
(deftemplate Basic-information
    ...
    (slot breakfast (default 1)) ; 早餐是否规律（1：规律，0：跳过）
    (slot lunch (default 1)) ; 午餐是否规律（1：规律，0：跳过）
    (slot dinner (default 1)) ; 晚餐是否规律（1：规律，0：跳过）
    (slot sleep-hours (default 8)) ; 平均睡眠时长，单位小时
    (slot stay-up (default 0)) ; 是否熬夜（1：是，0：否）
    (slot sleep-pattern (default nil)) ; 夜间睡眠模式（如醒来次数、睡眠时间段）
...
(deftemplate Match
    ...
    (slot similarity-score (default 0)) ; 计算的相似度得分
